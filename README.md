...
PeekDesktop 将 macOS Sonoma 的「点击壁纸显示桌面」功能带到 Windows 10 和 11。默认情况下，它使用资源管理器的原生「显示桌面」行为，同时还包含可选的「飞离」实验性效果，以及托盘菜单中的「需要双击」、「点击桌面时显示桌面」和「点击任务栏时显示桌面」开关。你可以正常点击或拖拽桌面图标，不会意外触发显示桌面。显示桌面后，再次点击任意应用窗口、任务栏空白区域或桌面壁纸，所有窗口就会恢复到原来的位置。
...
| 文件 | 适用平台 |
|------|----------|
| `PeekDesktop-vX.Y.Z-win-x64.zip` | Intel/AMD（大多数电脑） |
| `PeekDesktop-vX.Y.Z-win-arm64.zip` | ARM64（Surface Pro X、Snapdragon 等） |

无需安装程序。下载 zip 压缩包，解压后运行 `PeekDesktop.exe` 即可。发布版本为**自包含**单文件，无需额外安装 .NET。程序会驻留在系统托盘中，并**自动更新自身** —— 当新版本可用时，它会自动下载、验证代码签名，并原地重启完成更新。

## 文档

- **[工程深度解析](Docs/PeekDesktop-Engineering-Deep-Dive.md)** —— 架构、Shell 内部机制、实验、调试流程、未公开 API 说明及发布权衡
- **[自动更新器](Docs/Auto-Updater.md)** —— 原地自动更新系统的工作原理、安全模型、文件替换机制及测试说明

## 工作原理

1. **点击桌面空白壁纸或任务栏空白区域**（不是图标或任务栏按钮）-> 显示桌面
2. **停留在桌面上** -> 可以点击或拖拽图标、右键菜单、整理文件，窗口保持隐藏
3. **再次点击任意应用、任务栏或空白壁纸** -> 所有窗口恢复到原来的位置
